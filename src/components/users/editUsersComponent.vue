        ...
        <v-row no-gutters class="px-4" justify="space-around">
            ...
            <v-col cols="12" md="8">
                <v-row no-gutters>
                    ...
                    <v-col cols="12">
                        <ValidationProvider name="password" rules="min:6|confirmed:confirmation"
                                            v-slot="{ errors }">
                            <v-text-field
            ...
            <v-col cols="12" md="6">
                <v-dialog
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            ¿Estás seguro que deseas actualizar esta información?
                        </v-card-text>
                        ...
                        <v-card-actions>
                            ...
                            <v-btn color="warning" text @click="passes(_updateUser)" :loading="loading" :disabled="loading">
                                Editar Usuario
                            </v-btn>
                        </v-card-actions>
                    </v-card>
...
export default {
    ...
    methods: {
        ...
        ...mapActions('users', ['updateUser','showUser']),
        ...
        _getUser(){
          this.showUser(this.$route.params.id).then(response=>{
              this.data = {
                  ...
                  identificacion: response.data.data.identificacion,
                  correo: response.data.data.correo,
                  estado: response.data.data.estado,
                  password: null,
                  password_confirmation: null,
                  cambio_password: response.data.data.reinicio_password,
                  rol: null,
                  id: response.data.data.id
              };
              if (response.data.data.relaciones.roles.length > 0){
                  this.data.rol =  response.data.data.relaciones.roles[0].id;
              }
          }).catch(e=>{
              this.loader()
              console.log(e.response)
        ...
        _updateUser() {
            ...
            this.updateUser(this.data).then(response => {
                this.loading = false;
                this.dialog = false;
                this.dSuccess('Usuario editado con éxito')
